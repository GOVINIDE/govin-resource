...
const GovinExtension = require('./extension');
const {
    DEFAULT_HOST,
    DEFAULT_PORT,
    SERVER_NAME,
...
class ResourceServer extends Emitter{
    ...
    constructor (cacheResourcesPath, builtinResourcesPath) {
        ...
        this.extensions = new GovinExtension();
        this.devices = new GovinDevice();
        ...
        this._server.listen(this._port, this._host, () => {
            ...
            .on('error', err => {
                this.isSameServer('127.0.0.1', this._port).then(isSame => {
                    if (isSame) {
                        console.log(`Port is already used by other govin-resource server, will try reopening after ${REOPEN_INTERVAL} ms`); // eslint-disable-line max-len
                        setTimeout(() => {
                            this._server.close();
                            this._server.listen(this._port, this._host);
                        }, REOPEN_INTERVAL);
